 ...
 * <p>
 * Known issues:
 * - Files are not cleaned up properly when watching liveTV and comskip is started immediately.
 * - srt files are not cleaned up.  (filename.ts.CC1.srt, filename.ts.CC2.srt, etc.)
 *
 */
public class plugin implements sage.SageTVPlugin, SageTVEventListener {

    private final String                VERSION = "1.15 for testing.";
    ...
    public static final String          SETTING_COMSKIP_LOCATION    = "ComskipLocation";
    public static final String          PROPERTY_COMSKIP_LOCATION   = "cd/comskip_location";    
    ...
    public static final String          PROPERTY_SERVER_IS      = "cd/server_is";
    public static final String          PROPERTY_SERVER_ARCH    = "cd/server_arch";
    
    public static final String          SETTING_NATIVE_COMSKIP  = "NativeComskip";
    public static final String          PROPERTY_NATIVE_COMSKIP = "cd/native_comskip";  
    
    public static final String          PROPERTY_ONLY_QUEUE_FAVORITES   = "cd/only_favorites";
    ...
    public void start() {
        ...
        if (Global.IsWindowsOS()) {
            Configuration.SetServerProperty(PROPERTY_SERVER_IS, "windows");
        } else if (Global.IsLinuxOS()) {
            Configuration.SetServerProperty(PROPERTY_SERVER_IS, "linux");
        } else {
            Configuration.SetServerProperty(PROPERTY_SERVER_IS, "mac");
        }
        
        String serverIs = Configuration.GetServerProperty(PROPERTY_SERVER_IS, "windows");
        Log.getInstance().write(Log.LOGLEVEL_TRACE, "start: Server is " + serverIs);
        
        // Determine the underlying system architecture (32 or 64 bit). Only needed for now Windows systems.
        if (!serverIs.equals("windows")) {
            String arch = System.getProperty("os.arch");    // Will probably be x86 or xmd64.
            Configuration.SetServerProperty(PROPERTY_SERVER_ARCH, arch);
            Log.getInstance().write(Log.LOGLEVEL_TRACE, "start: Server architecture is " + arch);
            
            Log.getInstance().write(Log.LOGLEVEL_TRACE, "start: Making comskip executable.");
            
            String comskipLocation = Configuration.GetServerProperty(SETTING_COMSKIP_LOCATION, getDefaultComskipLocation() + File.separator + "comskip" + File.separator + "comskip.exe");
            ...
            if (arch.contains("64"))
                MakeExecutable.init(comskipLocation.replace(".exe", "64"));
            else
                MakeExecutable.init(comskipLocation.replace(".exe", "32"));
            }
        }
    ...
    public void destroy() {
        ...
        CSC.getInstance().destroy();
        SystemStatus.getInstance().destroy();
    ...
    public String[] getConfigSettings() {
        ...
        CommandList.add(SETTING_COMSKIP_LOCATION);
        ...
        String serverOS = Configuration.GetServerProperty(PROPERTY_SERVER_IS, "unknown");
        ...
        if (serverOS.equalsIgnoreCase("linux")) {
            ...
            CommandList.add(SETTING_NATIVE_COMSKIP);
            
            // See if we need to worry about setting up for wine.
            if (!SageUtil.GetBoolProperty(PROPERTY_NATIVE_COMSKIP, false)) {
    ...
    public String getConfigValue(String setting) {
        ...
        if (setting.startsWith("LogLevel")) {
            ...
            return n.toString();
        } else if (setting.startsWith(SETTING_COMSKIP_LOCATION)) {
            return Configuration.GetServerProperty(PROPERTY_COMSKIP_LOCATION, getDefaultComskipLocation() + File.separator + "comskip" + File.separator + "comskip.exe");
        ...
        } else if (setting.startsWith(SETTING_NATIVE_COMSKIP)) {
            return Configuration.GetServerProperty(PROPERTY_NATIVE_COMSKIP, "false");
    ...
    public void setConfigValue(String setting, String value) {
        ...
        if (setting.startsWith("LogLevel")) {
            ...
            Configuration.SetServerProperty(PROPERTY_COMSKIP_LOCATION, value);
        } else if (setting.startsWith("IniLocation")) {
            ...
            Configuration.SetServerProperty(PROPERTY_NATIVE_COMSKIP, value);
        } else if (setting.startsWith("Restart")) {
            ComskipManager.getInstance().startMaxJobs();
        } else if (setting.startsWith("StopAll")) {
            ComskipManager.getInstance().stopAll();
    ...
    public String getConfigHelpText(String setting) {
        if (setting.startsWith("LogLevel")) {
            ...
            return "If set to false then 'wine' will be used to run Windows comskip.exe.";
        } else if (Names.contains(setting)) {
    ...
    public final void resetConfig() {
        ...
        Configuration.SetServerProperty(PROPERTY_COMSKIP_LOCATION, getDefaultComskipLocation() + File.separator + "comskip" + File.separator + "comskip.exe");
        ...
        Configuration.SetServerProperty(PROPERTY_NATIVE_COMSKIP, "false");
    }
    ...
    private static void manualRun(String FileName) {
        ...
        Log.getInstance().write(Log.LOGLEVEL_ERROR, "manualRun: Manual run for " + FileName);
    ...
    */
    ...
    public synchronized void sageEvent(String eventName, java.util.Map eventVars) {
        ...
        if (eventName.startsWith("MediaFileRemoved")) {
            ...
            // Special feature for Jeff. Only queue Favorites.
            if (Configuration.GetServerProperty(plugin.PROPERTY_ONLY_QUEUE_FAVORITES, "false").equals("true") && !AiringAPI.IsFavorite(MediaFile)) {
                Log.getInstance().write(Log.LOGLEVEL_TRACE, "sageEvent: MediaFile is not a Favorite, skipping " + MediaFileAPI.GetMediaTitle(MediaFile));
                return;
            }
